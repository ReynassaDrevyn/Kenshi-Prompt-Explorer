...
function Restart-InStaIfNeeded {
    ...
    if (-not $PSCommandPath) {
        throw 'KenshiPromptExplorerPS5 must run in STA mode. Re-run it with your PowerShell host using -STA -File KenshiPromptExplorerPS5.ps1.'
    }
...
function Get-JsonHashtable {
    ...
    $converted = ConvertTo-HashtableRecursive -InputObject $parsed
...
function Show-FolderPicker {
    ...
    # Windows PowerShell 5.1 does not expose the newer, friendlier folder
    # picker used by later runtimes. Use an OpenFileDialog-based folder picker
    # so users get an Explorer-style view with an address bar/path box.
    ...
    if ($dialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        if ([System.IO.File]::Exists($selectedPath)) {
            return (Split-Path -Path $selectedPath -Parent)
        }
        if ([System.IO.Directory]::Exists($selectedPath)) {
            return $selectedPath
        }

        $parent = Split-Path -Path $selectedPath -Parent
